...
function showWelcomeTitle() {
    ...
    if (title) {
      ...
      }, { threshold: 0.2 }); // 20% видимості для спрацьовування

      observer.observe(title);
    }

    if (realm) {
        const observerRealm = new IntersectionObserver((entries, observer) => {
            ...
            if (entry.isIntersecting) {
              realmSpans.forEach(span => {
                ...
                const interval = setInterval(() => {
                  if (count <= 150) {
                    span.style.background = `linear-gradient(to right, rgba(0, 0, 0, 1) ${count - 50}%, rgba(0, 0, 0, 0) ${count}%)`;
        ...
        }, { threshold: 0.5 }); // 50% видимості для спрацьовування
